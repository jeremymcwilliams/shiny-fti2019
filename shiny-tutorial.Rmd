...
# drinks data frame
drinks=read.csv("drinks.csv")
#choices = setNames(drinks$total_litres_of_pure_alcohol, drinks$country)
choices = setNames(drinks$country, drinks$country)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("country", "Select your country",  choices=code)
    ),
...
server <- function(input, output) {
  ...
  output$selected_var <- renderPlot({
    ...
    barplot(c(beer, wine, spirits),names.arg=labels, main=input$country,  ylab="servings/year")
    #hist(c(5,6,3), main=input$country, xlab="Your selected number")
    #paste("You have selected", input$country)
  })
  
#  output$selected_country <- renderText({ 
#    paste("You have selected", input$val)
#  })
 
   
#  output$selected_range <-renderPlot({
#    hist(runif(input$country), main="test", xlab="Your selected number")
#  })
